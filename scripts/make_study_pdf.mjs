...
const markdown = `# Last Race Code Walkthrough Study Guide
...
function markdownToPdfLines(text) {
  const lines = [];
  for (const raw of text.split(/\r?\n/)) {
    if (raw.startsWith("# ")) {
      lines.push({ text: raw.slice(2), size: 18, bold: true, gapBefore: 8, gapAfter: 6 });
    } else if (raw.startsWith("## ")) {
      lines.push({ text: raw.slice(3), size: 14, bold: true, gapBefore: 10, gapAfter: 4 });
    } else if (raw.startsWith("- ")) {
      for (const part of wrapText(raw, 88)) {
        lines.push({ text: part, size: 9.5, bold: false, indent: 14 });
      }
    } else if (/^\d+\./.test(raw)) {
      for (const part of wrapText(raw, 88)) {
        lines.push({ text: part, size: 9.5, bold: false, indent: 14 });
      }
    } else if (raw.startsWith("Q:") || raw.startsWith("A:") || raw.startsWith("What to say:")) {
      for (const part of wrapText(raw, 92)) {
        lines.push({ text: part, size: 9.5, bold: raw.startsWith("Q:") || raw.startsWith("What to say:") });
      }
    } else if (raw.trim() === "") {
      lines.push({ text: "", size: 9.5, gapAfter: 5 });
    } else {
      for (const part of wrapText(raw, 94)) {
        lines.push({ text: part, size: 9.5, bold: false });
      }
    }
  }
  return lines;
  ...
  for (const [pageIndex, page] of pages.entries()) {
    ...
    for (const line of page) {
      ...
      if (line.text) {
        ...
        content += `BT /${font} ${line.size} Tf ${x} ${currentY} Td (${escapePdfText(line.text)}) Tj ET\n`;
      }
      currentY -= line.size + 4 + (line.gapAfter ?? 0);
    }
    content += `BT /F1 8 Tf ${pageWidth - margin - 70} 28 Td (Page ${pageIndex + 1} of ${pages.length}) Tj ET\n`;
...
writeFileSync(join(outDir, "LastRace_Code_Walkthrough.md"), markdown, "utf8");
writeFileSync(join(outDir, "LastRace_Code_Walkthrough.pdf"), makePdf(markdown));
console.log(`Created ${join(outDir, "LastRace_Code_Walkthrough.pdf")}`);
